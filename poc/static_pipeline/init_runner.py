...
def _plan_units(settings, client, model, ref, run_id, observer, max_steps=8) -> list[dict]:
    ...
    spec = AgentSpec(
        ...
        tools=_plan_tools(client, ref), state_schema=StaticState, run_id=run_id,
    ...
    units = extract_json_obj(text, "units").get("units", [])
...
def _reduce_and_save(
    *, settings, client, model, ref, run_id, repo_name, themes,
    summaries, observer, rev, summary,
) -> dict:
    ...
    def _gen_theme(theme: str):
        ...
        def factory(_t=theme, no_tools=False):
            return build_repo_writer_graph(
                ...
                ref=ref, summaries_block=summ_block, run_id=run_id,
                no_tools=no_tools,
            )
        ...
        path = save_theme_doc(out_dir, theme, doc_md)
        ...
        return theme, path, verdict, warned

    with concurrent.futures.ThreadPoolExecutor(
            max_workers=min(_REDUCE_CONCURRENCY, len(themes))) as ex:
        futs = {ex.submit(_gen_theme, t): t for t in themes}
        for fut in concurrent.futures.as_completed(futs):
            t = futs[fut]
            try:
                theme, path, verdict, warned = fut.result()
                summary["docs"][theme] = {
                ...
                summary["docs"][t] = {"error": f"{type(e).__name__}: {e}"}
                rev("engine_call", f"repo:{t}", "failed",
    ...
    rev("run", "static-init", "done",
        detail={"units": len(summary.get("units", [])),
...
def run_init(
    ...
    *,
    ...
    out_dir = settings.out_path / "init"
    out_dir.mkdir(parents=True, exist_ok=True)
    summary: dict = {"run_id": run_id, "ref": None, "units": [], "docs": {}, "warned": []}
    ...
    try:
        ...
        try:
            repo_name = client._client.get(
                f"{client.base}/projects/{client.project}").json().get("name", repo_name)
        ...
        cache_path = settings.out_path / "init" / "_summaries.json"
        if reuse_summaries and cache_path.exists():
            ...
            if data.get("ref") == ref and data.get("summaries"):
                ...
                return _reduce_and_save(
                    settings=settings, client=client, model=model, ref=ref,
                    run_id=run_id, repo_name=repo_name, themes=themes,
                    summaries=summaries, observer=observer, rev=rev, summary=summary,
                )
        ...
        summaries = map_unit_summaries(
            model=model, client=client, ref=ref, run_id=run_id, units=units,
            observer=observer, emit_ctx=rev,
            cache_path=out_dir / "_summaries.json", reuse=reuse_summaries,
        )
        if not summaries:
            rev("run", "static-init", "failed", detail={"error": "단위 요약 전멸"})
            return summary
        ...
        rev("run", "static-init", "failed", detail={"error": f"{type(e).__name__}: {e}"})
        raise
    finally:
        client.close()
        observer.close()
