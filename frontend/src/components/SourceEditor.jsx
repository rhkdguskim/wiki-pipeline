...
export function SourceEditor({form, onChange, onSave, onVerify, onTrigger, busy, message, verifyResult}) {
  ...
  return <form className="editor" onSubmit={ev => { ev.preventDefault(); onSave(); }}>
    ...
    <div className="formGrid">
      ...
      <label>{isGithub ? 'Repo (owner/repo)' : 'Project ID'}<input value={fieldValue(form, 'project_id')} onChange={e => set('project_id', e.target.value)} placeholder={isGithub ? 'owner/repo' : '947'} /></label>
      ...
      <label>Release branch<input value={fieldValue(form, 'release_branch')} onChange={e => set('release_branch', e.target.value)} placeholder="비우면 default branch" /></label>
