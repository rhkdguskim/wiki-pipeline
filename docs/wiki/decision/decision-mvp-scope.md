...
> **부분 번복 (2026-07-07)**: 위 "SCM 커넥터 = GitLab 1개, GitHub는 MVP 이후" 절단선은
> [[decision-scm-multi-instance-github-mvp]]로 **대체**됐다 — GitHub 커넥터와 SCM 다중 인스턴스
> (사내 GitLab·gitlab.com·github.com)를 MVP로 승격. **두 파이프라인 포함 절단선 자체는 여전히 유효**하며,
> 아래 "포함"·"제외"의 나머지 항목도 그대로다. GitHub 관련 항목(포함의 SCM 줄·제외의 첫 항목)만 갱신됐다.
...
- SCM 커넥터 — 추상화 인터페이스는 유지 → [[decision-scm-connector-abstraction]]. ~~MVP 구현은 GitLab 1개~~ **(2026-07-07 부분 번복 → SCM 다중 인스턴스 + GitHub 커넥터 MVP 승격)** → [[decision-scm-multi-instance-github-mvp]]. 실측 완료 환경은 [[entity-mirero-gitlab]]

## 제외

- ~~**GitHub 커넥터** — 인터페이스는 있으나 구현은 MVP 이후 (실측 환경이 GitLab뿐).~~ **(2026-07-07 번복 — GitHub 커넥터 + SCM 다중 인스턴스를 MVP로 승격)** → [[decision-scm-multi-instance-github-mvp]]
