...
> ⛔ **SUPERSEDED (2026-07-10)** by [[decision-monday-ingest-polling-only]]. 지연 목표가 "일 배치(오늘 데이터를 내일 문서에 반영)"로 확정([[question-dwh-latency-target]])되어 webhook 레인(실시간 근사)이 순수 과잉이 되었다. 이제 **스케줄러 폴링 단일 레인**만 운영한다. 아래 야간 전수 폴링 레인·lifecycle 복원 서술은 그 단일 레인으로 계승되며, webhook 레인만 삭제된다. webhook+폴링 하이브리드의 일반 패턴은 [[concept-readonly-saas-cdc]]에 보존돼, 지연 목표가 근실시간으로 바뀌면 복귀 대상이다.
>
> **이전 정정 노트 (2026-07-10, supersede 전)**: "bronze 착륙 방식"이 정정된 바 있음 — 원문의 `bronze.monday_webhook_events_raw` 전용 테이블 + idempotent **upsert**는 단일 테이블 `raw_records` + **append(sha256 동일 시 skip)**로 대체됨([[decision-bronze-single-jsonb-table]], [[decision-dwh-as-karpathy-llm-wiki]]).
...
2. **야간 전수 폴링 레인(진실 보정)** — 매일 02:00 KST에 Monday GraphQL API를 `items_page` cursor(500/page)로 전수 순회, boards/groups/columns/users/teams/tags는 전수 refresh, updates/activity_logs는 `from_date` 증분. 결과를 같은 bronze 원본 테이블에 upsert(`extracted_at`·`extraction_batch_id` 추가). **이 폴링 레인의 실행체는 스케줄러가 트리거하는 LangGraph 수집 에이전트다 → [[decision-monday-collector-langgraph-scheduled]]. 모든 Monday 접근은 읽기 전용 래퍼를 통과 → [[decision-monday-readonly-client-wrapper]].**
...
- [[decision-monday-ingest-polling-only]] — ⛔ 이 결정을 supersede (webhook 삭제, 폴링 단일 레인)
...
- [[decision-monday-readonly-client-wrapper]] — 두 레인 모두 읽기 전용 래퍼 경유
- [[decision-bronze-single-jsonb-table]] — bronze 착륙 방식 정정(upsert 전용테이블 → append 단일테이블)
- [[decision-dwh-as-karpathy-llm-wiki]] — 정정의 상위 근거(Karpathy raw 불변 축적)
