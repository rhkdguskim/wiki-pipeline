...
- **러너 → AI API 네트워크 경로 확보됨** ✅ (2026-07-05 확인, 폐쇄망 차단 아님) → [[question-runner-ai-network]]. 인증/실행 방식도 확정됨(2026-07-06) — API 키 + 자체 에이전트 → [[decision-engine-api-key-auth]] · [[decision-engine-api-agent]]
...
- **CodeScene 정적분석**: `mirero/Static-Code-Analysis` 그룹 26개 프로젝트(`pwm-*-codescene` 등) — 코드 인덱스와의 중복 우려는 파이프라인 범위 제외로 해소 → [[question-blob-vs-code-index-overlap]].
