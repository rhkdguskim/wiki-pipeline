...
python -m backend.common.run --smoke

# ① 정적 파이프라인 (compare -> 테마별 문서 생성)
python -m backend.static_pipeline.main            # .env의 sha 사용
python -m backend.static_pipeline.main --from <sha> --to <sha>
# 산출: backend/out/{theme}.md + backend/out/events-*.jsonl

# 관측 대시보드 API (정적 서빙 없음 — JSON API만)
python -m backend.dashboard.serve                 # http://127.0.0.1:8420/api
...
python -m backend.manual_pipeline.main            # 시나리오 + 자율 탐색 -> user/operator 매뉴얼
python -m backend.manual_pipeline.main --no-explore --themes user-manual
python -m backend.manual_pipeline.main --resume manual-xxxxxxxx   # 체크포인트 중단 재개 (L4)
# 산출: backend/out/manual/{theme}.md + observations-*.jsonl + coverage-*.json + shots/
```
...
- 설정/토큰: `backend/.env`의 `DOCSHUB_*`와 control-plane DB(`backend/out/control-plane.sqlite`)에 보관한다. API 응답에는 토큰 값을 반환하지 않고 `has_token`만 노출한다.
...
전부 `backend/.env`(gitignore 차단). 코드·문서 어디에도 하드코딩하지 않는다.
