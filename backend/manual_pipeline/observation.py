...
class ObservationLog:
    ...
    def record(self, *, tool: str, args: dict, ok: bool, preview: str) -> Observation:
        obs = Observation(
            seq=len(self.items) + 1, phase=self.phase, tool=tool, args=args,
            ok=ok, preview=preview[:_PREVIEW_CHARS],
    ...
    def evidence_block(self, max_chars: int = 80000) -> str:
        ...
        def build(cut: int | None) -> str:
            lines = []
            for o in self.items:
                a = json.dumps(o.args, ensure_ascii=False)
                if len(a) > 160:
                    a = a[:160] + "…"
                p = o.preview if cut is None else o.preview[:cut]
