...
(프롬프트·도구·테마 데이터·근거 소스)은 전부 콜러블/데이터로 주입받는다.
  ...
  - evidence_builder       : LLM 호출 전 근거를 구조화된 evidence pack 으로
  - scope_planner          : evidence 기반 문서 생성/수정/스킵 결정
  - deterministic_verifier : LLM critic 전 기계 검증 (frontmatter·근거·secret 등)
  - grounding_critic       : chunk 분할 critic (9k 단절 해결) + verdict schema
  - quality_gates          : verifier + critic 결과로 quality/terminal status 결정
"""
